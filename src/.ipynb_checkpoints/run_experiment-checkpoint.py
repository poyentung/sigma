...
class DoExperiments(object):
    def __init__(self, descriptor:str, general_results_dir:str,
                 autoencoder:torch, model_args:dict,
                 learning_rate:float, weight_decay:float,
                 num_epochs:int, patience:int, batch_size:int,
                 debug:bool, task:str, old_model_params_dir:str, 
                 chosen_dataset:Dataset, chosen_dataset_args:dict ):
        ...
        if self.task in ['predict_on_test']:
            self.old_model_params_dir = old_model_params_dir
        
        
        #Data 
        if self.task  == 'train_eval':
            self.dataset_train = chosen_dataset('train',**chosen_dataset_args)
            self.dataset_test = chosen_dataset('test',**chosen_dataset_args)
        elif self.task  == 'train_all':
            self.dataset_train = chosen_dataset('all',**chosen_dataset_args)
    ...
    def run_model(self):
        self.model = self.autoencoder(**self.model_args).to(self.device)
